...
namespace NiobiumStudios
{
    ...
    **/
    public class TimedRewards : DailyRewardsCore<TimedRewards>
    {
        ...
        private float maxTime = 3600f; // How many seconds until the player can claim the reward
        public float minRandomTime = 3600f; // How many minimum seconds until the player can claim the reward
        public float maxRandomTime = 7200f; // How many maximum seconds until the player can claim the reward
        ...
        void Start()
        {
            maxTime = UnityEngine.Random.Range(minRandomTime, maxRandomTime);
        ...
        void LoadTimerData ()
        {
            string lastRewardTimeStr = EncryptedPlayerPrefs.GetString(GetTimedRewardsTimeKey());
        ...
        public void CheckCanClaim ()
        {
            ...
            else
            {
                ...
                EncryptedPlayerPrefs.SetString(GetTimedRewardsTimeKey(), now.Add(timer - TimeSpan.FromSeconds(maxTime)).ToString(FMT));
            }
        }

        //Returns the TimeRewardsTime EncryptedPlayerPrefs key depending on instanceId
        ...
        public void Reset()
        {
            EncryptedPlayerPrefs.DeleteKey(GetTimedRewardsTimeKey());
            canClaim = true;
