        ...
        foreach (RaycastHit2D raycastHit2D in raycastHit2DArray) {
            if (raycastHit2D.collider != null) {
                ...
                if (!colliderList.Contains(raycastHit2D.collider)) {
                    ...
                    if (raycastHit2D.collider.gameObject.GetComponent<PlayerMobileInput>() != null) {
                        // Hit an Item
                        radarPing.SetColor(new Color(0, 1, 0));
                    }
                    if (raycastHit2D.collider.gameObject.GetComponent<EnemyController>() != null || raycastHit2D.collider.gameObject.GetComponent<HeliCopterEnemyBehaviour>() != null) {
                        // Hit an Enemy
